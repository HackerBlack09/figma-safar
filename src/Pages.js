...
function Pages({ search1 }) {
  useEffect(() => {
    Aos.init();
  }, []);
  const [bolean, SetBolean] = useState(0);
  const { sizemassiv, savatfun, likefun, colormassiv, category, local, btn } =
            ...
            {local
              ...
              .filter((obj) => {
                if (search1 === "") {
                  ...
                  obj.text
                    .toLowerCase()
                    .includes(search1.toLowerCase().trim()) ||
                  obj.text
                    .toLowerCase()
                    .includes(search1.toLowerCase().trim()) ||
                  obj.price.toString().includes(search1.toLowerCase().trim())
                ) {
                  return obj;
                }
              })
